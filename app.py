...
def word2vec_bigram(datanew, d, w, m):
    ...
    w2v_bigram_model = Word2Vec(bigrams[datanew], vector_size=d, window= w, min_count=m, workers=4)
...
def w2v_tfidf(Text, datanew, d, w, m):
    ...
    w2v_tfidf_model = Word2Vec(datanew, min_count=m, vector_size=d,window= w, workers = 4)
...
def upload_amazon():
    ...
    no_clusters = 6
    dataset = pd.read_excel('app/AmazonReviews.xlsx')
...
def upload_nih():
    ...
    no_clusters = 4
    # C:\Users\Admin\Desktop\New folder\trial\Flask-html updated\nih.xlsx
    data = pd.read_excel('nih.xlsx')
    data['text'] = data['Description'].combine_first(data['Topics'])
    data.to_csv('app/file.csv')
...
def word2vecresult(no):
    ...
    plt.savefig('app/static/images/w2v_hyper_dimension.jpg')
    plt.clf()
    ...
    plt.savefig('app/static/images/w2v_hyper_window.jpg')
    plt.clf()
...
def word2vecAnalysis(no):
    ...
    w2vtfidf_kmeans_pie.savefig('app/static/images/w2vtfidf_kmeans_pie.jpg')
    ...
    w2vpca_tfidf_kmeans_pie.savefig('app/static/images/w2vpca_tfidf_kmeans_pie.jpg')
    ...
    w2vpca_tfidf_h_pie.savefig('app/static/images/w2vpca_tfidf_h_pie.jpg')
    ...
    plott.scatter(centers[:, 0], centers[:, 1], c='black', s=200, alpha=0.5);
    plott.savefig('app/static/images/scatter.jpg')
    plott.clf()
...
def lsaresult(no):
    ...
    lsa_spectral_pie.savefig('app/static/images/lsa_spectral_pie.jpg')
    ...
    lsa_db_pie.savefig('app/static/images/lsa_db_pie.jpg')
    lsa_db_pie.clf()
...
def NMFresult(no):
    ...
    nmf_pie.savefig('app/static/images/nmf_pie.jpg')
    nmf_pie.clf
...
def ldaresult(no):
    ...
    lda_pie.savefig('app/static/images/lda_pie.jpg')
    lda_pie.clf()
...
def ldawords(no):
    ...
    p = pyLDAvis.gensim_models.prepare(topic_model=lda_model, corpus=corpus, dictionary=dictionary_LDA)
    pyLDAvis.save_html(p, 'app/templates/ldaWords.html')
...
def doc2vect(no):
    ...
    d2v_pie.savefig('app/static/images/d2v_pie.jpg')
    d2v_pie.clf()
...
def w2v_data():
    return flask.send_file('app/file_w2v.csv',
                     mimetype='text/csv',
...
def lda_data():
    return flask.send_file('app/file_lda.csv',
                     mimetype='text/csv',
...
def d2v_data():
    return flask.send_file('app/file_d2v.csv',
                     mimetype='text/csv',
